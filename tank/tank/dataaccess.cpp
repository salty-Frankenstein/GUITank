...
/* class NormalDataAccess */
NormalDataAccess::NormalDataAccess() {}

wstring NormalDataAccess::GetFile(string path) {
	return stringToLPCWSTR(path);
}

void NormalDataAccess::ClearFile() {}

/* class CompressedDataAccess */
CompressedDataAccess::CompressedDataAccess(string path, string name)
	:da(path, name), prefix(path + name) {}

wstring CompressedDataAccess::GetFile(string path) {
	for (int i = 1; i <= prefix.length(); i++)
		path.erase(path.begin());
	path = "." + path;
	da.Load(path);
	return stringToLPCWSTR(DataAccessor::GetLastPath(path));
}

void CompressedDataAccess::ClearFile() {
	da.Clear();
}
